...
const CommanderDashboard = observer(({ userId }) => {
...
// ב־CommanderDashboard
useEffect(() => {
  const storedUserId = Number(localStorage.getItem("userId"));
  if (storedUserId) {
    userStore.loadUsers().then(() => {
      const foundUser = toJS(userStore.filterById(storedUserId))[0] || null;
      setUser(foundUser);
    });
  }
}, []);
      ...
      >
        ...
        <ul className="nav flex-column">
          ...
          <li className="nav-item">
           ...
           <li className="nav-item">
            <Link to="mission-add" className="nav-link text-white" onClick={() => setIsSidebarOpen(false)}>
              הוסף משימה
            </Link>
          </li>
      ...
      >
        ...
        <nav className="navbar navbar-expand-lg navbar-light bg-light border-bottom d-none d-md-flex">
          <div className="container-fluid">
            <span style={{ marginRight: "250px" }} className="navbar-brand"> {user?.first_name} המפקד</span>
          </div>
        </nav>
